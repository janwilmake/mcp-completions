...
export interface OAuthProvider extends Record<string, SqlStorageValue> {
  id: string;
  /** The resource URL pattern this auth applies to */
  resource_url: string;
...
async function discoverProtectedResourceMetadata(
  ...
  // Priority 1: Use resource_metadata from WWW-Authenticate header
...
async function registerClientWithCIMD(
  ...
  // Construct CIMD URL from client info
  const cimdUrl = `${clientInfo.uri}/oauth/client-metadata.json`;

  // Return the CIMD URL as client_id
  return {
    clientId: cimdUrl,
  };
}
...
export async function constructAuthorizationUrl(
  ...
  // Use scope from WWW-Authenticate or fall back to scopes_supported
  if (!scope && resourceMetadata.scopes_supported) {
  ...
  // Register client - prefer CIMD, fall back to dynamic registration
  let clientId: string;
  let clientSecret: string | undefined;

  if (authMetadata.client_id_metadata_document_supported && clientInfo.uri) {
    // Use Client ID Metadata Document
    ...
    const refreshPromises = expiredProviders.map(async (provider) => {
      try {
        ...
        await stub.updateTokens(
          ...
          tokenData.refresh_token || provider.refresh_token || undefined,
...
export async function getAuthorizationForUrl(
  ...
  try {
    ...
    const provider = await stub.findProviderForUrl(resourceUrl);
    ...
    if (
      ...
      if (now >= expirationTime - 300) {
        try {
          ...
          return {
            Authorization: `${provider.token_type || "Bearer"} ${tokenData.access_token}`,
          };
        } catch (error) {
          // Refresh failed, fall through to return existing token or login URL
        }
      }
    }
    ...
    // Return login URL if no valid auth
    const baseUrl = options.baseUrl || "https://example.com";
    const pathPrefix = options.pathPrefix || "/oauth";
    return {
      loginUrl: `${baseUrl}${pathPrefix}/login?url=${encodeURIComponent(resourceUrl)}`,
    };
  } catch (error) {
    // Return login URL on error
    const baseUrl = options.baseUrl || "https://example.com";
    const pathPrefix = options.pathPrefix || "/oauth";
    return {
      loginUrl: `${baseUrl}${pathPrefix}/login?url=${encodeURIComponent(resourceUrl)}`,
